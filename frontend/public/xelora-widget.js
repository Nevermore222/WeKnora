/**
 * Xelora embed widget SDK — floating chat launcher.
 *
 * Programmatic:
 *   Xelora.init({ channel, token, position, primaryColor, title, baseUrl })
 *   Xelora.open() | close() | toggle() | destroy()
 *   Xelora.on('ready', fn) | off('ready', fn)
 *
 * Host context & actions (postMessage to iframe):
 *   Xelora.setContext({ userId, page, ... })
 *     Inject visitor/page context merged into each chat query.
 *   Xelora.openWithQuery('How do I reset my password?')
 *     Opens the panel (if closed) and sends the query when the iframe is ready.
 *   Xelora.setLocale('en-US')
...
(function (global) {
  ...
  var HOST_SOURCE = 'xelora-host';
  var EMBED_SOURCE = 'xelora-embed';
  ...
  function emit(event, payload) {
    ...
    handlers.slice().forEach(function (fn) {
      try { fn(payload); } catch (e) { console.error('[Xelora]', e); }
    });
  }
  ...
  function createWidget(opts) {
    ...
    if (!channelId || (!staticToken && !tokenEndpoint)) {
      console.warn('[Xelora] channel and (token or tokenEndpoint) are required');
      return null;
    }
    ...
    function loadToken(force) {
      ...
      tokenInFlight = fetch(tokenEndpoint, {
        ...
        .catch(function (e) {
          console.error('[Xelora] failed to load token', e);
          throw e;
        })
    ...
    if (!baseUrl) {
      ...
      if (script && script.src) {
        baseUrl = script.src.replace(/\/xelora-widget\.js.*$/, '');
      } else {
    ...
    function postHostPayload(type, payload) {
      if (!iframe.contentWindow) {
        console.warn('[Xelora] iframe not ready');
        return false;
      }
    ...
    function setContext(ctx) {
      if (!ctx || typeof ctx !== 'object') {
        console.warn('[Xelora] setContext expects an object');
        return;
      }
    ...
    function openWithQuery(query) {
      ...
      if (!text) {
        console.warn('[Xelora] openWithQuery requires a non-empty query');
        return;
      }
      setOpen(true);
    ...
    function setLocale(locale) {
      ...
      if (!loc) {
        console.warn('[Xelora] setLocale requires a locale string');
        return;
      }
