...
instance.interceptors.request.use(
  (config) => {
    ...
    // 但这条优化会被任何把 xelora_tenant 写成激活租户的代码（OIDC
...
instance.interceptors.response.use(
  ...
  async (error: any) => {
    ...
    if (error.response.status === 401 && !originalRequest._retry && !originalRequest.url?.includes('/auth/refresh')) {
      ...
      const refreshToken = localStorage.getItem('xelora_refresh_token');
      ...
      if (refreshToken) {
        try {
          ...
          if (response.success && response.data) {
            ...
            localStorage.setItem('xelora_token', token);
            localStorage.setItem('xelora_refresh_token', newRefreshToken);
