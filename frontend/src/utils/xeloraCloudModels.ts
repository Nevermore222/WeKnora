...
export const XELORA_CLOUD_BASE_URL = 'https://xelora.weixin.qq.com'
export const XELORA_CLOUD_PROVIDER = 'xeloracloud'
...
export function isXeloraCloudModel(model: ModelConfig): boolean {
  return model.parameters?.provider === XELORA_CLOUD_PROVIDER
}

/** Returns kinds that already have a XeloraCloud model of the matching backend type. */
export function existingWkcKinds(models: ModelConfig[]): Set<WkcModelKind> {
  const found = new Set<WkcModelKind>()
  for (const model of models) {
    if (!isXeloraCloudModel(model)) continue
...
export function buildWkcModelConfig(
  ...
  const parameters: ModelConfig['parameters'] = {
    base_url: XELORA_CLOUD_BASE_URL,
