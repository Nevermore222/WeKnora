<template>
  <div class="xeloracloud-settings">
    <div class="section-header">
      <h2>{{ $t('settings.xeloraCloud.title') }}</h2>
      <p class="section-description">
        {{ $t('settings.xeloraCloud.description') }}
        <a
          class="doc-link"
        ...
        >
          {{ $t('settings.xeloraCloud.viewDocs') }}
    ...
    <div v-if="credentialState === 'unconfigured'" class="credential-status unconfigured">
      ...
      <span>{{ $t('settings.xeloraCloud.unconfigured') }}</span>
    </div>
    ...
    <div v-else-if="credentialState === 'expired'" class="credential-warning">
      ...
      <div class="warning-text">
        <strong>{{ $t('settings.xeloraCloud.expired') }}</strong><br />
        {{ reinitReason || $t('settings.xeloraCloud.expiredDefault') }}
      </div>
    </div>
    ...
    <div v-else-if="credentialState === 'configured'" class="credential-status success">
      ...
      <span class="status-text">{{ $t('settings.xeloraCloud.configured') }}</span>
      <t-button
        v-if="!formExpanded"
        variant="outline"
        theme="default"
      ...
      >
        ...
        {{ $t('settings.xeloraCloud.reconfigure') }}
      </t-button>
    </div>
    ...
    <div v-if="formExpanded" class="settings-group">
      <div class="setting-row">
        <div class="setting-info">
          <label class="setting-label">{{ $t('settings.xeloraCloud.appIdLabel') }}</label>
          <p class="setting-desc">{{ $t('settings.xeloraCloud.appIdDesc') }}</p>
        </div>
        <div class="setting-control">
          <t-input
            v-model="form.appId"
            :placeholder="$t('settings.xeloraCloud.appIdPlaceholder')"
            autocomplete="off"
            style="width: 280px;"
          />
        </div>
      ...
      <div class="setting-row">
        <div class="setting-info">
          <label class="setting-label">{{ $t('settings.xeloraCloud.appSecretLabel') }}</label>
          <p class="setting-desc">{{ $t('settings.xeloraCloud.appSecretDesc') }}</p>
        </div>
        <div class="setting-control">
          <t-input
            v-model="form.appSecret"
            type="password"
            :placeholder="$t('settings.xeloraCloud.appSecretPlaceholder')"
      ...
      <div class="setting-row action-row">
        <div class="setting-info">
          <p class="setting-desc">{{ $t('settings.xeloraCloud.saveHint') }}</p>
        </div>
        <div class="setting-control">
          <t-button
            theme="primary"
            ...
            @click="handleSave"
          >
            {{ $t('settings.xeloraCloud.saveBtn') }}
          </t-button>
        </div>
      </div>
    </div>
    ...
    >
      <div class="models-section__header">
        <h3 class="models-section__title">{{ $t('settings.xeloraCloud.modelsSection.title') }}</h3>
        <p class="models-section__desc">
          {{
            credentialState === 'configured'
              ? $t('settings.xeloraCloud.modelsSection.descReady')
              : $t('settings.xeloraCloud.modelsSection.descPending')
          }}
        </p>
      </div>
        ...
        >
          ...
          <div class="model-row__action">
            <t-tag
              ...
              size="small"
            >
              {{ $t('settings.xeloraCloud.modelsSection.statusAdded') }}
            </t-tag>
            <t-popconfirm
              v-else-if="credentialState === 'configured'"
              :content="$t('settings.xeloraCloud.modelsSection.confirmAddOne', {
                type: kindLabel(kind),
                name: WKC_MODEL_NAME_BY_KIND[kind],
              })"
              :confirm-btn="{ content: $t('settings.xeloraCloud.modelsSection.addOne'), theme: 'primary' }"
            ...
            >
              <t-button
                ...
                @click.stop
              >
                {{ $t('settings.xeloraCloud.modelsSection.addOne') }}
              </t-button>
            </t-popconfirm>
            <span v-else class="model-row__pending">
              {{ $t('settings.xeloraCloud.modelsSection.statusPending') }}
            </span>
          </div>
        </div>
      </div>
      ...
      >
        <t-popconfirm
          :content="$t('settings.xeloraCloud.modelsSection.confirmAddAll', { count: missingKinds.length })"
          :confirm-btn="{
            content: $t('settings.xeloraCloud.modelsSection.addAllConfirm'),
            theme: 'primary',
          }"
          :cancel-btn="{ content: $t('common.cancel') }"
          placement="top"
        ...
        >
          ...
          >
            {{ $t('settings.xeloraCloud.modelsSection.addAllBtn', { count: missingKinds.length }) }}
          </t-button>
        </t-popconfirm>
      </div>
      ...
      >
        ...
        {{ $t('settings.xeloraCloud.modelsSection.allReady') }}
      </p>
    </section>

    <!-- 使用说明 -->
    <div class="usage-hint">
      <p class="hint-title">{{ $t('settings.xeloraCloud.usageTitle') }}</p>
      <p class="hint-text" v-html="$t('settings.xeloraCloud.usageSteps').replace(/\n/g, '<br />')" />
    </div>
  </div>
</template>
...
import { saveXeloraCloudCredentials, getXeloraCloudStatus, createModel, listModels } from '@/api/model'
import { testEmbeddingModel } from '@/api/initialization'
import {
  WKC_MODEL_KINDS,
  WKC_MODEL_NAME_BY_KIND,
  XELORA_CLOUD_BASE_URL,
  XELORA_CLOUD_PROVIDER,
  buildWkcModelConfig,
  existingWkcKinds,
  type WkcModelKind,
} from '@/utils/xeloraCloudModels'
...
const kindDisplayName = (kind: WkcModelKind) =>
  t(`settings.xeloraCloud.addModelsDisplayName.${kind}`)
...
const resolveEmbeddingDimension = async (): Promise<number> => {
  const result = await testEmbeddingModel({
    ...
    baseUrl: XELORA_CLOUD_BASE_URL,
    provider: XELORA_CLOUD_PROVIDER,
  })
  if (!result.available || !result.dimension) {
    throw new Error(result.message || t('settings.xeloraCloud.addModelsEmbeddingFailed'))
  }
  ...
  try {
    for (const kind of targets) {
      try {
        ...
        console.error(`Failed to create XeloraCloud ${kind} model:`, err)
        failed += 1
      }
    }

    if (success > 0 && failed === 0) {
      MessagePlugin.success(t('settings.xeloraCloud.addModelsSuccess', { count: success }))
    } else if (success > 0) {
      MessagePlugin.warning(t('settings.xeloraCloud.addModelsPartial', { success, failed }))
    } else {
      MessagePlugin.error(t('settings.xeloraCloud.addModelsFailed'))
    }
  } finally {
...
const handleSave = async () => {
  if (!form.value.appId || !form.value.appSecret) {
    MessagePlugin.warning(t('settings.xeloraCloud.fillRequired'))
    return
  }
  saving.value = true
  try {
    await saveXeloraCloudCredentials({
      app_id: form.value.appId,
      app_secret: form.value.appSecret,
    })
    MessagePlugin.success(t('settings.xeloraCloud.saveSuccess'))
    form.value.appId = ''
    ...
    MessagePlugin.error(err?.message || t('settings.xeloraCloud.saveFailed'))
  } finally {
    saving.value = false
  }
}
...
const checkStatus = async () => {
  try {
    const status = await getXeloraCloudStatus()
