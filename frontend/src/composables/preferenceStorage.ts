 ...
 *
 * Storage layout: Xelora_${userId}_${suffix}, where userId is the active
 ...
 *   - Legacy un-namespaced keys (Xelora_${suffix}) from earlier branch
...
export function readUserId(): string {
  try {
    const raw = localStorage.getItem('xelora_user')
    if (!raw) return 'anon'
...
export function safeSetItem(key: string, value: string): void {
  try {
    ...
    console.warn(`[Xelora] failed to persist preference "${key}":`, err)
  }
}
...
export function userKey(suffix: string): string {
  return `Xelora_${readUserId()}_${suffix}`
}
...
export function migratePreferencesIntoUser(): void {
  ...
  for (const suffix of PREFERENCE_SUFFIXES) {
    const target = `Xelora_${userId}_${suffix}`
    const targetExists = safeGetItem(target) !== null

    const anonKey = `Xelora_anon_${suffix}`
    const legacyKey = `Xelora_${suffix}`
