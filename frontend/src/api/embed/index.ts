...
const EMBED_MSG_SOURCE = 'xelora-embed'
const EMBED_HOST_SOURCE = 'xelora-host'
...
export function buildWidgetSnippet(
  ...
  const attrs = [
    `src="${escapeHtmlAttr(`${base}/xelora-widget.js`)}"`,
...
export const SECURE_TOKEN_ENDPOINT_PLACEHOLDER = 'https://your-backend.example.com/xelora/embed-token'
...
export function buildSecureServerNodeExample(channelId: string, opts?: { baseUrl?: string }) {
  ...
  return [
    `// Node/Express — keep XELORA_PUBLISH_TOKEN only on the server (env var).`,
...
export function buildSecureServerGoExample(channelId: string, opts?: { baseUrl?: string }) {
  ...
  const exchangeUrl = `${base}/api/v1/embed/${channelId}/exchange`
  return [
    `// Go net/http — keep XELORA_PUBLISH_TOKEN only on the server (env var).`,
