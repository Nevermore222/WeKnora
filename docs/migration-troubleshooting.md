...
This guide is linked from the system info page when Xelora's startup database
...
Xelora auto-runs `golang-migrate` migrations on every startup. When a
...
Then restart Xelora. The next startup will pick up where the failing
migration left off.
...
default, Xelora's startup tries to auto-recover; if you disabled that with
...
4. **Fix the underlying cause** (install extension, fix privileges, free
   disk, …), then either:
   - Restart Xelora and let auto-recovery retry; **or**
...
<https://github.com/Tencent/Xelora/issues/new?template=bug_report.yml>

Include:

- Xelora version + commit ID (from the system info page).
