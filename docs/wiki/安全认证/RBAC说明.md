...
本文档介绍 Xelora 的**租户内权限控制（Tenant RBAC）**，包括角色矩阵、资源归属模型，以及它与 [共享空间](./共享空间说明.md) 的关系。
...
环境变量 `XELORA_TENANT_ENABLE_RBAC` / `XELORA_AUDIT_RETENTION_DAYS` 覆盖 YAML。`DISABLE_REGISTRATION=true` 等价于把 `registration_mode` 强制设为 `invite_only`。
