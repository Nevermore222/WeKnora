...
Xelora 支持通过 Langfuse 对 Agent 的 ReAct 循环、大模型 Token 消耗、工具调用以及异步任务流水线进行全链路追踪。
...
如需临时回退到「仅审计、不拦截」灰度窗口，可在配置里设置 `tenant.enable_rbac=false`（或环境变量 `XELORA_TENANT_ENABLE_RBAC=false`）。完整的角色矩阵和归属链请见 [`docs/RBAC说明.md`](./RBAC说明.md)。
...
0.6.2 起，文件 / URL / 文件夹上传可携带 `process_config`（`KnowledgeProcessOverrides`），在**本次批次**内覆盖知识库默认的解析引擎、分块、多模态（VLM / ASR）、问题生成、图谱抽取等设置，而不会改动 KB 全局配置。Web UI 在上传前会弹出确认对话框供调整；API 与 `xelora doc upload` 传同名 JSON 即可。
...
## 19. 升级到 0.6.2 后 `xelora` CLI 登录或 MCP 工具报错？

0.6.2 随附 **CLI v0.9**（破坏性变更），常见迁移：

- **`auth login` 不再创建 profile**：先 `xelora profile add <name> --host <url> --use`，再 `xelora auth login`；切换 profile 用全局 `--profile <name>`。
...
- 新增 `xelora session stop <session-id>` 可中止进行中的 Agent 运行；仓库内附带 `xelora-rag-search` / `xelora-shared` 内置 Skills。
