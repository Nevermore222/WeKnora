...
本文档介绍 Xelora **租户内权限控制（Tenant RBAC）** 的设计、角色矩阵、资源归属模型、配置方式，以及它与 [共享空间](./共享空间说明.md) 之间的关系。
...
| 环境变量 | YAML 路径 | 取值 |
|----------|-----------|------|
| `XELORA_TENANT_ENABLE_RBAC` | `tenant.enable_rbac` | `true` / `false` |
| `XELORA_AUDIT_RETENTION_DAYS` | `audit.retention_days` | 非负整数 |
...
```bash
export XELORA_TENANT_ENABLE_RBAC=false
# 重启服务即可回到观察模式
```
...
<table>
  ...
  <tr>
    ...
    <td width="50%" align="center">
      ...
      <br/><sub>任何用户都可以自助创建租户，创建后自动成为新空间的 Owner（受 <code>XELORA_TENANT_MAX_PER_USER</code> 上限保护）。</sub>
    </td>
  </tr>
  <tr>
    <td colspan="2" align="center">
