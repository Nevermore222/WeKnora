...
DocReader 是 Xelora 项目中负责文档解析和处理的 gRPC 服务。它支持多种文档格式的读取、OCR 识别、多模态处理等功能。
...
```yaml
docreader:
  image: wechatopenai/xelora-docreader:${XELORA_VERSION:-latest}
  environment:
...
- `MINIO_BUCKET_NAME`: MinIO 存储桶名称（默认：Xelora）
...
docker logs Xelora-docreader
```

## 更多信息

- 服务端口：50051（gRPC）
- 容器名称：Xelora-docreader
- 网络：Xelora-network
- 重启策略：unless-stopped
