...
[![Artifact Hub](https://img.shields.io/endpoint?url=https://artifacthub.io/badge/repository/xelora)](https://artifacthub.io/packages/helm/xelora/xelora)
[![License](https://img.shields.io/badge/License-MIT-blue.svg)](https://opensource.org/licenses/MIT)

Helm chart for deploying [Xelora](https://github.com/Tencent/Xelora) - an AI-powered Knowledge RAG Platform.

## Overview

Xelora is an intelligent knowledge base platform that combines:
...
helm install xelora ./helm \
  ...
  --set ingress.host=xelora.example.com \
  --set ingress.tls.enabled=true \
  --set ingress.tls.secretName=xelora-tls \
  --set secrets.dbPassword=secure-password \
  --set secrets.redisPassword=secure-password \
  --set secrets.jwtSecret=$(openssl rand -base64 32)
```
...
ingress:
  enabled: true
  host: xelora.company.com
  tls:
    enabled: true
    secretName: xelora-tls

secrets:
  existingSecret: xelora-secrets  # Use pre-created secret
```
...
| `app.image.repository` | Image repository | `wechatopenai/xelora-app` |
...
| `frontend.image.repository` | Image repository | `wechatopenai/xelora-ui` |
...
| `ingress.host` | Hostname | `xelora.example.com` |
...
| `secrets.dbName` | Database name | `xelora` |
...
1. **Helm --set flags** (for testing)
   ```bash
   helm install xelora ./helm --set secrets.dbPassword=xxx
   ```

2. **External Secrets Operator** (recommended for production)
   ```yaml
   secrets:
     existingSecret: xelora-external-secret
   ```
...
```bash
helm uninstall xelora --namespace xelora

# Optional: Remove PVCs
kubectl delete pvc -n xelora -l app.kubernetes.io/instance=xelora
```
...
### Check Pod Status
```bash
kubectl get pods -n xelora
```

### View Logs
```bash
# Backend logs
kubectl logs -n xelora -l app.kubernetes.io/component=app -f

# Frontend logs
kubectl logs -n xelora -l app.kubernetes.io/component=frontend -f
```

### Common Issues

**Pod stuck in Pending**
- Check if PVCs are bound: `kubectl get pvc -n xelora`
- Verify storage class exists: `kubectl get sc`

**Connection refused errors**
- Wait for all pods to be Ready
- Check service endpoints: `kubectl get endpoints -n xelora`

**Database connection errors**
- Verify secrets are correct
- Check PostgreSQL logs: `kubectl logs -n xelora -l app.kubernetes.io/component=database`

## Contributing

See [CONTRIBUTING.md](https://github.com/Tencent/Xelora/blob/main/CONTRIBUTING.md) in the main repository.
...
This chart is licensed under the MIT License - see the [LICENSE](https://github.com/Tencent/Xelora/blob/main/LICENSE) file for details.
