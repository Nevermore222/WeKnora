...
// Package e2e_test drives the Xelora CLI binary against a real running
...
//
//	cd cli
//	XELORA_E2E_HOST=https://kb.example.com \
//	XELORA_E2E_TOKEN=eyJhbGc... \
//	go test -tags=acceptance_e2e -v ./acceptance/e2e/...
//
// Optional XELORA_E2E_KB_NAME_PREFIX customizes the throwaway KB name (default
...
func TestRAGFullLoop(t *testing.T) {
	host := mustEnv(t, "XELORA_E2E_HOST")
	token := mustEnv(t, "XELORA_E2E_TOKEN")
	prefix := envOr("XELORA_E2E_KB_NAME_PREFIX", "cli-e2e-")
...
// CLI finds a profile without needing `xelora profile add` (which prompts
...
func writeSampleDoc(t *testing.T) string {
	t.Helper()
	content := `Xelora E2E Sample Document

This sample document is used by the Xelora CLI acceptance test suite to
validate the end-to-end retrieval-augmented generation pipeline.

向量检索的核心思想是把文本通过 embedding 模型映射到高维向量空间,然后通过余弦相似度
等度量找出语义最接近的内容片段。Xelora 支持 vector + keyword 的混合检索模式。
...
// indexing is complete. Xelora server uses a few status values across versions
