...
func TestDiscover_FoundInCwd(t *testing.T) {
	...
	want := filepath.Join(dir, ".xelora", "project.yaml")
	if got != want {
...
func TestLoad_RoundTrip(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, ".xelora", "project.yaml")
...
func TestSave_CreatesParent(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".xelora")); err != nil {
		t.Errorf("parent .xelora not created: %v", err)
	}
