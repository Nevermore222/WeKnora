...
// names the bundled `xelora-shared` skill advertises (in its "CLI vs MCP"
...
func TestSkillReferencedMCPToolsExist(t *testing.T) {
	...
	skillPath := filepath.Join(filepath.Dir(file), "..", "..", "skills", "xelora-shared", "SKILL.md")
	...
	if start < 0 {
		t.Fatal("xelora-shared SKILL.md has no 'CLI vs MCP' section to validate")
	}
	section := body[start:]
	...
	for name := range referenced {
		if !registered[name] {
			t.Errorf("xelora-shared skill references MCP tool %q which is NOT registered (renamed/removed?)", name)
		}
	}
}
