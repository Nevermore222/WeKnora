// Package skillparity contains the K6 drift guard: every xelora command and
...
import (
	...
	"github.com/Tencent/Xelora/cli/cmd"
...
func TestSkillsReferenceLiveCommandsAndFlags(t *testing.T) {
	...
	for _, path := range files {
		...
		for _, block := range codeFence.FindAllStringSubmatch(content, -1) {
			for _, line := range strings.Split(block[1], "\n") {
				idx := strings.Index(line, "xelora ")
				if idx < 0 {
					continue
				}
				inv := line[idx+len("xelora"):]
				...
				// Placeholder command (e.g. `xelora <command> --help`) → skip.
	...
	if !checkedAny {
		t.Fatal("parser resolved no xelora commands from skills — check tokenizer")
	}
}
