...
// closure is lazy: --help / completion / `xelora version` must NOT trigger
// HTTP, keyring access, or filesystem I/O beyond the bare minimum.
//
// Four closures:
//   - Config:   parses ~/.config/xelora/config.yaml (no network)
...
//
// Client returns a *sdk.Client (the Xelora SDK). Commands that want narrow
...
func buildClient(f *Factory) (*sdk.Client, error) {
	...
	if profileName == "" {
		return nil, NewError(CodeAuthUnauthenticated, "no current profile configured; run `xelora auth login` to set one up")
	}
	prof, ok := cfg.Profiles[profileName]
	if !ok {
		// If the user explicitly overrode the profile (via --profile flag or
		// XELORA_PROFILE env), it's a bad argument - not a corrupt config file.
		...
		if f.ProfileOverride != "" {
			return nil, NewError(CodeInputInvalidArgument,
				...
				WithHint("list available profiles with `xelora profile list`").
				WithRetryCommand("xelora profile list")
		}
...
//  2. XELORA_KB_ID env (always an explicit id)
//  3. .xelora/project.yaml (walk-up from cwd)
//  4. error: kb required
...
// ApplyLogLevel resolves --log-level / XELORA_LOG_LEVEL (in priority order)
...
func (f *Factory) ActiveProfile() string {
	...
	if v := os.Getenv("XELORA_PROFILE"); v != "" {
		return v
	}
	if f.Config == nil {
		return ""
