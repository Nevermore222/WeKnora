...
func TestSaveLoad_RoundTrip(t *testing.T) {
	...
	in := &Config{
		...
		Profiles: map[string]Profile{
			"prod": {Host: "https://kb.example.com", TenantID: 7, APIKeyRef: "keychain://xelora/prod/access"},
		},
	}
...
func TestLoad_Corrupt(t *testing.T) {
	...
	p := filepath.Join(dir, "xelora", "config.yaml")
...
func TestPath_HonorsXDG(t *testing.T) {
	...
	p, err := Path()
	require.NoError(t, err)
	assert.Equal(t, filepath.Join(xdg, "xelora", "config.yaml"), p)
}
...
func TestPath_FallsBackToHome(t *testing.T) {
	...
	assert.Contains(t, filepath.ToSlash(p), "/.config/xelora/config.yaml")
}
...
func TestSave_AtomicReplace(t *testing.T) {
	...
	matches, _ := filepath.Glob(filepath.Join(dir, "xelora", "*.tmp"))
