# Changelog — `xelora` CLI

All notable changes to the `xelora` CLI (the binary under `cli/` in this
...
of the Xelora server / frontend release cadence.
...
#### Added
- `xelora session stop <session-id>` command to abort an in-flight agent run.
...
- **`auth login` drops `--host` and `--name`.** It authenticates the active profile; create it first with `xelora profile add <name> --host <h> --use`. Target a non-active profile with the global `xelora --profile <name> auth login`.
...
#### Added
- `--dry-run` flag on every mutation cobra command (`kb create/edit/delete`, `agent create/edit/delete`, `doc create/upload/fetch/delete`, `chunk delete`, `session delete`, `auth refresh/logout`, `link/unlink`, `profile add/remove`) and on `xelora api` (POST/PUT/PATCH/DELETE only; GET returns FlagError exit 2).
- envelope `meta.dry_run: true` + `meta.plan: {action, args | method+path+body}` open-map fields (omitempty in non-dry-run envelopes).
- `xelora session continue-stream <session-id> --message <msg-id>` command for SSE event stream replay/recovery.
...
#### Changed
- `SetAgentHelp` wrapper in `cli/internal/cmdutil/agenthelp.go` now prepends "Risk:" line in default (non-JSON) help branch when `cmd.Annotations["risk.action"]` is set. XELORA_AGENT_HELP=1 JSON path unchanged.
...
- **`--format` default flips to `json` regardless of TTY.**
  ...
  - Migration: humans on a TTY pass `--format text` (or set
    `XELORA_FORMAT=text` env) for the prior auto-text behavior.
...
- **`xelora agent invoke` removed; use `xelora session ask --agent <id>`.**
  - Server route is `POST /sessions/{session_id}/agent-qa` — session-anchored.
  - `xelora agent` keeps CRUD only (list / view / create / edit / delete /
    status / check).
  - Migration: `xelora agent invoke ag_x "Q"` →
    `xelora session ask --agent ag_x "Q"` (auto-creates session if none given).
- **`xelora doc upload` split into three commands.**
  - `xelora doc upload <file>` — local file only.
  - `xelora doc fetch <url>` — server-side remote fetch (was `upload --from-url`).
  - `xelora doc create --text "..."` — direct text knowledge.
...
- **`xelora kb empty` removed; use `xelora doc delete --all --kb=<id>`.**
  - Atomic server `ClearKnowledgeBaseContents` (no list-then-delete race).
  - Same exit-10 `-y/--yes` guard as `kb delete`.
  - Migration: `xelora kb empty kb_x -y` →
    `xelora doc delete --all --kb=kb_x -y`.
- **`xelora api -d/--data` flag removed; use `--input <file>` or `--input -`
  (stdin).**
  - `xelora api` now accepts any non-empty HTTP method (whitelist removed)
    so the escape hatch can hit endpoints the CLI doesn't natively model.
  - Migration: `xelora api -d '{"foo":1}' /endpoint` →
    `echo '{"foo":1}' | xelora api --input - /endpoint`.
- **Batch operations envelope shape — per-item `ok` pattern.**
  - `xelora doc delete id1,id2,id3` and similar multi-id mutations now emit:
...
- **Unknown subcommand emits typed envelope.**
  - `input.unknown_subcommand` with `detail.{unknown, command_path, available[]}`
    ...
    `"unknown command \"x\" for \"xelora\""` prose.
- **`xelora chat` requires the query as a single quoted argument.**
  - v0.6: `MinimumNArgs(1)` silently joined `xelora chat hello world` into
    `"hello world"`.
  - v0.7: `ExactArgs(1)` rejects multi-arg with exit 2; matches
    `xelora session ask`. Quote the query: `xelora chat "hello world"`.

#### Added
- **`XELORA_PROFILE` env var** selects the active profile for a single
...
- **`XELORA_FORMAT` env var** sets the default `--format`. Values:
...
- **`error.retry_after_seconds`** — `server.rate_limited` / `server.timeout`
  surface server `Retry-After` header verbatim. CLI-direct (`xelora api`)
...
- **`xelora doc fetch <url>`** — new command (see split above).
- **`xelora doc create --text "..."`** — new command (see split above).
- **`xelora session ask --agent <id> "..."`** — new command (replaces
  `agent invoke`).
- **`xelora doc delete --all --kb=<id>`** — new mode of `doc delete`
  (replaces `kb empty`).
...
- **`XELORA_SDK_DEBUG=1` env removed** → use **`XELORA_LOG_LEVEL=debug`**.
...
- **`xelora doc wait <id> [<id>...]`** — block until every document reaches a
...
- **`--log-level error|warn|info|debug`** persistent flag + `XELORA_LOG_LEVEL`
...
- **`xelora kb status <id>`** — fast health snapshot (1 HTTP). Returns
  reachable / counts / is_processing.
- **`xelora kb check <id>`** — deep verification: status fields + `failed_count`
...
- **`SetAgentHelp` pattern** — `cmdutil.SetAgentHelp(cmd, AgentHelp{...})`
  ...
  shape, activated by `XELORA_AGENT_HELP=1` at `--help` time. Applied
...
#### Added
- `xelora agent create <name> --model <id>` / `agent edit <id>` /
...
- `xelora chunk list --doc <doc-id>` / `chunk view <chunk-id>` /
  `chunk delete <chunk-id> --doc <doc-id>` — new subtree for RAG retrieval
  debug. Paginated with v0.4 `--limit` / `--page-size` / `--all-pages` canon.
- `xelora mcp serve` adds `chunk_list` as the 10th curated tool.
- `xelora agent view <id>` human output now renders all 34 AgentConfig
...
- `xelora doc list` gains `--keyword` / `--file-type` / `--source` /
...
- `xelora session view --full` (with `--limit`, default 50, bounds
...
- `xelora kb view` human render now includes `TYPE`, `PINNED` (badge,
...
- `xelora doc view` human render expands to include `TITLE` (when
...
- `xelora doc upload` gains `--enable-multimodel` (tri-state:
...
#### Breaking changes
- `xelora search docs` now applies the keyword filter server-side via
  ...
  **The match is now case-sensitive** (server uses `LIKE %keyword%`),
  ...
  back to `xelora api` with a custom filter.
...
- `cli/README.md` now documents the `xelora api` raw HTTP passthrough
...
#### Added
- `xelora mcp serve` — curated read-only stdio MCP server exposing 9
...
- `xelora agent list` / `agent view` / `agent invoke` — manage and
  call Xelora's server-side Custom Agent resources.
- `xelora auth token` — print the active credential to `stdout` for
  scripting (raw secret by default; `--json` emits `{token, mode, context}`).
- `xelora doc upload --from-url` — ingest a remote URL.
...
- `auth login --with-token` validates the supplied key against
  ...
  `$XDG_CONFIG_HOME/xelora/secrets/`.
...
- `unlink` — remove the cwd's `.xelora/project.yaml` so subsequent
