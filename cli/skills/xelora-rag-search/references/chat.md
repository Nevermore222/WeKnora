...
```
xelora chat "<query>" --kb <name-or-id> [--session <id>]
xelora session ask "<query>" --agent <agent-id> [--session <id>]
```
...
- **Stop server-side generation:** `xelora session stop <session-id> --message
  <message-id>`. Ctrl-C only closes your local connection — the server keeps
  generating (and billing) until told to stop.
- **Re-attach after a dropped connection:** `xelora session continue-stream
