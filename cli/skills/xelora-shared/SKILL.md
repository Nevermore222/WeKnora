---
name: xelora-shared
description: Use when driving a Xelora RAG server through the `xelora` CLI as an agent — authenticating, managing knowledge bases / documents / sessions / agents, running search or chat, or interpreting the CLI's JSON envelopes and exit codes. Read this before any other xelora-* skill.
metadata:
  tested_against: v0.9
---

# Xelora CLI — shared base

`xelora` is the agent-first CLI for a Xelora RAG server. Every command prints
a JSON envelope and uses a typed exit code, so you branch on machine-readable
output, not prose. Read this skill before any task-specific `xelora-*` skill.

## 1. Authenticate (do this first — order matters)

Authentication is a **two-step sequence**. `xelora auth login` operates on the
...
echo "$XELORA_API_KEY" | xelora auth login --with-token
# 2b. OR email+password (interactive prompt only — no flags; not for agents)
xelora auth login

xelora auth status          # verify: who am I, which tenant
```

- Target a *non-active* profile for one command with the global `--profile NAME`
  (e.g. `xelora --profile staging auth refresh`). There is no per-command
  `--name`/`--host` on auth commands.
- Get the raw token for scripting with `XELORA_TOKEN=$(xelora auth token)`
  (raw token by default; `--format json` gives the `{token, mode, profile}` envelope).
- `xelora doctor` runs 4 health checks (reachability, credential, version, storage).

## 2. Selecting a knowledge base (`--kb`)

`--kb` accepts a **name or a UUID** (resolved server-side). Resolution order:
`--kb` flag → `XELORA_KB_ID` env → directory link (`xelora link --kb X` binds
...
- `--jq '<expr>'` filters the envelope (e.g. `xelora kb list --jq '.data[].id'`).
- Exception: `xelora auth token` emits the **raw token** by default (it's a
...
| 2 | flag / argument validation (bad/unknown/missing-required flag) | re-check `xelora <cmd> --help` |
...
{"ok":false,"error":{"type":"input.confirmation_required",
  ...
  "retry_command":"xelora kb delete X -y","risk":{"level":"destructive","action":"kb.delete"}}}
```
...
```bash
KB=$(xelora kb create "Docs" --jq '.data.id' --format json | tr -d '"')
xelora doc upload ./manual.pdf --kb "$KB"
```
...
link/unlink  bind cwd to a KB        mcp serve  expose xelora as MCP tools
```

**chat vs session ask vs search** — the most common confusion: see the
`xelora-rag-search` skill for the decision table. Briefly: `chat` = one-shot
...
For an IDE/host agent that speaks MCP, `xelora mcp serve` exposes a curated
...
Run `xelora <command> --help`. With `XELORA_AGENT_HELP=1` set, `--help` emits a
...
| Mistake | Fix |
|---|---|
| `xelora auth login` before any profile exists | `profile add <n> --host <url> --use` first (§1) |
...
| `xelora chat "..."` prints "garbled" stream | NDJSON by default; use `--format text` for a transcript |
| `search chunks "q"` → exit 1 `local.kb_id_required` | Pass `--kb <name-or-id>`, set `XELORA_KB_ID`, or `xelora link` the dir |
