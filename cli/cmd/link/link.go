// Package linkcmd implements `xelora link` - binds the current working
// directory to a knowledge base by writing .xelora/project.yaml. Always
...
import (
	...
	"github.com/Tencent/Xelora/cli/internal/projectlink"
)
...
// NewCmd builds the `xelora link` command.
func NewCmd(f *cmdutil.Factory) *cobra.Command {
	opts := &Options{}
	cmd := &cobra.Command{
		Use:   "link",
		Short: "Bind the current directory to a knowledge base",
		Long: `Writes .xelora/project.yaml in the current working directory pointing
...
overridden by the --kb flag or XELORA_KB_ID env var.
...
user explicitly asked to bind this directory; don't run it as a side effect.`,
		Example: `  xelora link --kb a32a63ff-fb36-4874-bcaa-30f48570a694    # explicit UUID
  xelora link --kb engineering                             # name → id
  xelora link                                              # interactive (TTY)`,
		Args: cobra.NoArgs,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor:       "Bind the current directory to a knowledge base by writing .xelora/project.yaml. Requires --kb (non-interactive); only run when the user explicitly asks to link this directory.",
...
// per-invocation override flag on `xelora link` itself - to record under a
// different profile, use the global persistent flag (`xelora --profile
...
func resolveProfile(f *cmdutil.Factory) (string, error) {
	...
	if cfg.CurrentProfile == "" {
		return "", cmdutil.NewError(cmdutil.CodeAuthUnauthenticated, "no active profile; run `xelora auth login` first")
	}
