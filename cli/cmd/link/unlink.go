...
// NewCmdUnlink builds `xelora unlink`. Walks up from cwd so a user in
...
func NewCmdUnlink() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Removes the .xelora/project.yaml that ` + "`xelora link`" + ` previously
...
is present anywhere in the parent chain.`,
		Example: `  xelora unlink           # remove the binding for this project
  xelora unlink --format json    # bare JSON (CI / agents)`,
		Args: cobra.NoArgs,
		RunE: func(c *cobra.Command, _ []string) error {
			...
			if !found {
				return &cmdutil.Error{
					...
					Hint:    "run `xelora link --kb <id>` first, or check you're in the right directory",
				}
			}
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor: "Remove the .xelora/project.yaml KB binding from the current directory tree. No flags required; walks up from cwd to find the link.",
