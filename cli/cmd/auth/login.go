...
// beforehand with `xelora profile add <name> --host <h> --use`.
...
// NewCmdLogin builds the `xelora auth login` command. runF is the testable
...
func NewCmdLogin(f *cmdutil.Factory, runF func(context.Context, *LoginOptions, *cmdutil.FormatOptions, *cmdutil.Factory, LoginService) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "login",
		Short: "Authenticate the active profile against its Xelora server",
...
` + "`xelora profile add <name> --host <h> --use`" + `, then run ` + "`xelora auth login`" + `.

Credentials are persisted to the OS keyring when available; otherwise to a
0600 file under $XDG_CONFIG_HOME/xelora/secrets.`,
		Args: cobra.NoArgs,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{`echo "$XELORA_API_KEY" | xelora auth login --with-token`},
		Output:   "envelope.data is {profile, host, mode, user, tenant_id} on success",
		Warnings: []string{
			"a profile must exist and be active first (`xelora profile add <n> --host <url> --use`)",
...
func resolveActiveProfile(f *cmdutil.Factory) (name, host string, err error) {
	// f.Config() already folds the global --profile / XELORA_PROFILE override
	...
	if active == "" {
		return "", "", cmdutil.NewError(cmdutil.CodeAuthUnauthenticated,
			"no active profile; run `xelora profile add <name> --host <h> --use` first")
	}
...
func runLogin(ctx context.Context, opts *LoginOptions, fopts *cmdutil.FormatOptions, f *cmdutil.Factory, svc LoginService) error {
	...
	if opts.WithToken {
		...
		if err != nil {
			...
			// get CodeNetworkError and the hint points at `xelora doctor`.
...
func warnOnFileFallback(store secrets.Store) {
	...
	fmt.Fprintln(iostreams.IO.Err, "warning: OS keychain unavailable - credentials will be saved to a 0600 file under $XDG_CONFIG_HOME/xelora/secrets/.")
	fmt.Fprintln(iostreams.IO.Err, "         install / unlock the keyring (or use `xelora doctor` to inspect) for OS-backed storage.")
}
