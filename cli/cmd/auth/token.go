...
// NewCmdToken builds `xelora auth token`. Prints the active profile's
// credential to stdout for use in shell pipelines, e.g.
//
//	XELORA_TOKEN=$(xelora auth token)
//	curl -H "Authorization: Bearer $XELORA_TOKEN" ...     # JWT mode
//	curl -H "X-API-Key: $XELORA_TOKEN" ...                # api-key mode
//
...
(mode: bearer), depending on how the profile was created. Run ` + "`xelora auth list`" + `
...
` + "`--profile <name>`" + ` (global flag) selects a non-active profile to read from.`,
		Example: `  XELORA_TOKEN=$(xelora auth token)
  xelora auth token --profile staging
  xelora auth token --format json`,
		Args: cobra.NoArgs,
...
func runToken(f *cmdutil.Factory, fopts *cmdutil.FormatOptions) error {
	...
	default:
		return cmdutil.NewError(cmdutil.CodeAuthUnauthenticated,
			fmt.Sprintf("profile %q has no stored credential; run `xelora auth login`", profileName))
	}

	if token == "" {
		return cmdutil.NewError(cmdutil.CodeAuthUnauthenticated,
			fmt.Sprintf("profile %q credential is empty in keyring; run `xelora auth login`", profileName))
	}
	...
	// No trailing newline - clean $(xelora auth token) substitution.
	...
	if iostreams.IO.IsStdoutTTY() {
		...
		fmt.Fprintln(iostreams.IO.Err, "hint: pipe to $(xelora auth token) to capture; this terminal scrollback now contains the secret")
		if mode == ModeAPIKey {
