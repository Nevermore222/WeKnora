...
// NewCmdLogout builds `xelora auth logout`. Clears stored credentials
...
them with --all) and drop the profile entry from ~/.config/xelora/config.yaml.
...
accepted until it expires.`,
		Example: `  xelora auth logout                       # active profile
  ...
  xelora auth logout --all`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			"xelora auth logout",
			"xelora --profile staging auth logout",
			"xelora auth logout --all",
		},
		Warnings: []string{
...
func runLogout(opts *LogoutOptions, fopts *cmdutil.FormatOptions, f *cmdutil.Factory) error {
	...
	retryCmd := "xelora auth logout -y"
	if opts.All {
		retryCmd = "xelora auth logout --all -y"
	}
