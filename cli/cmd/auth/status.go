...
// NewCmdStatus builds the `xelora auth status` command.
...
` + "`xelora auth login`" + ` (or ` + "`auth refresh`" + ` for JWT profiles) to recover.
...
usually only surfaces a hard auth failure.`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{"xelora auth status", "xelora auth status --jq .data.tenant_id"},
...
func runStatus(ctx context.Context, fopts *cmdutil.FormatOptions, f *cmdutil.Factory, svc StatusService) error {
	if svc == nil {
		return cmdutil.NewError(cmdutil.CodeAuthUnauthenticated, "no SDK client available; run `xelora auth login`")
	}
