// Package chat implements `xelora chat <text>` - the streaming RAG answer
// entry point.
//
...
// NewCmd builds `xelora chat <text>`.
func NewCmd(f *cmdutil.Factory) *cobra.Command {
	opts := &Options{}
	cmd := &cobra.Command{
		Use:   `chat "<text>"`,
		Short: "Ask a streaming RAG question against a knowledge base",
		Long: `Send a query to the Xelora knowledge-chat endpoint and stream the
...
Modes:
  ...
  --format json / --format ndjson / pipe (default): NDJSON event stream —
                                 ...
                                 stream.`,
		Example: `  xelora chat "What is RRF?" --kb a32a63ff-fb36-4874-bcaa-30f48570a694
  xelora chat "Summarise this design doc" --kb my-kb --format json
  xelora chat "Continue?" --session sess_abc`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples:      []string{`xelora chat "What is RRF?" --kb kb_abc --format json`},
...
func runChat(ctx context.Context, opts *Options, fopts *cmdutil.FormatOptions, svc ChatService) error {
	...
	autoCreated := false
	if sessionID == "" {
		sess, err := svc.CreateSession(ctx, &sdk.CreateSessionRequest{Title: "xelora chat"})
		if err != nil {
