...
// NewCmdDocs builds `xelora search docs "<query>" --kb <id-or-name>`.
...
` + "`xelora search docs \"$(printf %s YOUR_QUERY | tr 'A-Z' 'a-z')\"`" + `, or
fall back to ` + "`xelora api`" + ` with a custom filter.
...
reached or the KB is exhausted. Pass --all-pages=false to stop after one page.`,
		Example: `  xelora search docs "Q3 forecast" --kb finance
  xelora search docs "spec" --kb engineering --limit 5
  xelora search docs "spec" --kb engineering --all-pages=false`,
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			// `doc list` / `chat`), so a linked directory or XELORA_KB_ID
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor:       "Find documents in a knowledge base by keyword (server-side LIKE filter on title/file_name). The KB comes from --kb (id or name), else XELORA_KB_ID, else the linked directory. Results come with meta.count; use --limit to cap and --all-pages=false to stop after one page.",
		RequiredFlags: []string{"<query> (positional)", "--kb (or XELORA_KB_ID / linked directory)"},
		Examples:      []string{`xelora search docs "spec" --kb engineering --format json`},
