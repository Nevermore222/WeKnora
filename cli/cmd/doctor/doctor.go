// Package doctor implements `xelora doctor` - 4-item self-check.
//
...
import (
	...
	"github.com/Tencent/Xelora/cli/internal/compat"
...
func NewCmd(f *cmdutil.Factory) *cobra.Command {
	...
	cmd.Flags().BoolVar(&opts.NoCache, "no-cache", false, "Bypass server-info cache (located at $XDG_CACHE_HOME/xelora/server-info.yaml); force re-probe")
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			"xelora doctor",
			"xelora doctor --offline",
			"xelora doctor --no-cache --format json",
		},
...
func runChecks(ctx context.Context, opts *Options, svc Services, cliVer string) Result {
	...
	if !cascade(&checks[0], opts.Offline) {
		...
		if err := svc.PingBaseURL(ctx); err != nil {
			...
			checks[0].Hint = "verify the host configured for the active profile (run `xelora profile list` / `xelora profile add <n> --host=...`) and network reachability"
	...
	if !cascade(&checks[1], opts.Offline, &checks[0]) {
		if _, err := svc.GetCurrentUser(ctx); err != nil {
			...
			checks[1].Hint = "run `xelora auth login`"
...
func fillCredentialStorageCheck(c *Check) {
	...
	if _, isFile := store.(*secrets.FileStore); isFile {
		...
		c.Hint = "secrets persist at 0600 under $XDG_CONFIG_HOME/xelora/secrets/; install / unlock keyring for OS-backed storage"
		return
	}
	c.Status = StatusOK
...
// e.g. first-time `xelora doctor` to diagnose setup. Pre-resolving Client
...
func buildServices(f *cmdutil.Factory) (Services, error) {
	...
	// XELORA_BASE_URL still wins as a test/dev override; production reads host.
	if v := os.Getenv("XELORA_BASE_URL"); v != "" {
		host = v
	}
