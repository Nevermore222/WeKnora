...
// NewCmdDelete builds `xelora session delete`. Single-id keeps the simpler
...
without the user's explicit go-ahead.`,
		Example: `  xelora session delete s_abc                  # interactive confirm
  xelora session delete s_abc -y               # no prompt
  xelora session delete s_abc -y --format json # bare {id, deleted:true} JSON
  xelora session delete s_a s_b s_c -y         # delete 3, keep-going`,
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if err := cmdutil.ConfirmDestructiveBatch(f.Prompter(), opts.Yes, fopts.WantsJSON(), "delete", "session", len(args), "session.delete", "xelora session delete "+strings.Join(args, " ")+" -y"); err != nil {
				return err
			}
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			"xelora session delete s_abc -y",
			"xelora session delete s_a s_b s_c -y",
			"xelora session delete s_abc -y --format json",
		},
		Warnings: []string{
...
func runDelete(ctx context.Context, opts *DeleteOptions, fopts *cmdutil.FormatOptions, svc DeleteService, p prompt.Prompter, id string) error {
	if err := cmdutil.ConfirmDestructive(p, opts.Yes, fopts.WantsJSON(), "delete", "session", id, "session.delete", "xelora session delete "+id+" -y"); err != nil {
		return err
	}
