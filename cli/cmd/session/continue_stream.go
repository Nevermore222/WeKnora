// continue_stream.go implements `xelora session continue-stream` —
...
//
// Output shape matches `xelora chat` and `xelora session ask` NDJSON mode:
...
// NewCmdContinueStream builds `xelora session continue-stream <session-id> --message <id>`.
...
silently treated as NDJSON.`,
		Example: `  xelora session continue-stream sess_xyz --message msg_abc
  xelora session continue-stream sess_xyz -m msg_abc --format ndjson`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			"xelora session continue-stream sess_xyz --message msg_abc --format json",
