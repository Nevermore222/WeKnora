// stop.go implements `xelora session stop` — cancel server-side generation
...
// NewCmdStop builds `xelora session stop <session-id> --message <id>`.
...
Symmetric with 'session continue-stream': both key on (session_id, message_id).`,
		Example: `  xelora session stop sess_xyz --message msg_abc`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples:      []string{"xelora session stop sess_xyz --message msg_abc"},
