...
import (
	...
	"github.com/Tencent/Xelora/cli/internal/sse"
...
// NewCmdAsk builds `xelora session ask --agent <agent-id> "<text>"`.
...
The 'xelora agent' subtree handles CRUD only (list / view / create /
...
Modes:
  ...
  --format json / --format ndjson / pipe (default): NDJSON event stream —
                                 ...
                                 NDJSON stream.`,
		Example: `  xelora session ask --agent ag_x "Summarize Q3 sales"
  xelora session ask --session sess_x --agent ag_x "Follow-up question"
  xelora session ask --agent ag_x "Multi-step task" --format ndjson`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			`xelora session ask --agent ag_x "Summarize Q3 sales" --format json`,
			`xelora session ask --session sess_x --agent ag_x "Follow-up question" --format json`,
		},
...
func runAsk(ctx context.Context, opts *AskOptions, fopts *cmdutil.FormatOptions, svc AskService) error {
	...
	autoCreated := false
	if sessionID == "" {
		sess, err := svc.CreateSession(ctx, &sdk.CreateSessionRequest{Title: "xelora session ask"})
		if err != nil {
