// Package doc — fetch.go implements `xelora doc fetch <url>`.
package doc
...
// NewCmdFetch builds `xelora doc fetch <url>`.
...
--kb flag > XELORA_KB_ID env > .xelora/project.yaml > error.
...
Server-side ingestion knobs:
  ...
  --channel <name>         Override the ingestion-channel tag (default "api").`,
		Example: `  xelora doc fetch https://example.com/whitepaper.pdf
  xelora doc fetch https://example.com/no-ext --file-type pdf --title "Whitepaper"
  xelora doc fetch https://example.com/article.html --name "Q3 Article" --tag-id tag_abc
  xelora doc fetch https://example.com/report.pdf --kb my-kb --enable-multimodel`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor:       "Ingest a remote URL into the resolved knowledge base. KB resolved via --kb flag, XELORA_KB_ID env, or project link. Emits the created Knowledge object with its id.",
