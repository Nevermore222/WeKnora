...
// NewCmdDownload builds `xelora doc download <id>`. Positional id, output
...
stdout.`,
		Example: `  xelora doc download doc_abc                       # writes ./<server-name>
  xelora doc download doc_abc -O report.pdf
  xelora doc download doc_abc --output -            # stream to stdout (binary safe)
  xelora doc download doc_abc -O report.pdf --clobber
  xelora doc download doc_abc -O report.pdf --format json   # JSON envelope`,
