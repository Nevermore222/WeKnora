...
// NewCmdDelete builds `xelora doc delete`. Single-id keeps the simpler
...
without the user's explicit go-ahead.`,
		Example: `  xelora doc delete doc_abc                        # interactive confirm
  xelora doc delete doc_abc -y                     # no prompt
  xelora doc delete doc_abc -y --format json       # bare {id, deleted:true} JSON
  xelora doc delete doc_a doc_b doc_c -y           # delete 3, keep-going
  xelora doc delete doc_a doc_b --format json      # multi-id JSON output
  xelora doc delete --all --kb=kb_x -y             # clear all docs in kb_x
  xelora doc delete --all --kb=kb_x -y --format json # agent-friendly`,
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if opts.All {
				if opts.KB == "" {
					return cmdutil.NewError(cmdutil.CodeInputInvalidArgument, "--all requires --kb=<id>").
						...
						WithRetryCommand("xelora doc delete --all --kb=<kb-id> -y")
				}
				if len(args) > 0 {
			...
			if err := cmdutil.ConfirmDestructiveBatch(f.Prompter(), opts.Yes, fopts.WantsJSON(), "delete", "document", len(args), "doc.delete", "xelora doc delete "+strings.Join(args, " ")+" -y"); err != nil {
				return err
			}
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			"xelora doc delete doc_abc -y",
			"xelora doc delete doc_a doc_b doc_c -y",
			"xelora doc delete --all --kb=kb_x -y --format json",
		},
		Warnings: []string{
...
func runDelete(ctx context.Context, opts *DeleteOptions, fopts *cmdutil.FormatOptions, svc DeleteService, p prompt.Prompter, id string) error {
	if err := cmdutil.ConfirmDestructive(p, opts.Yes, fopts.WantsJSON(), "delete", "document", id, "doc.delete", "xelora doc delete "+id+" -y"); err != nil {
		return err
	}
...
func runDeleteAll(ctx context.Context, opts *DeleteOptions, fopts *cmdutil.FormatOptions, svc AllService, p prompt.Prompter) error {
	if err := cmdutil.ConfirmDestructive(p, opts.Yes, fopts.WantsJSON(), "delete", "all docs in KB", opts.KB, "doc.delete_all", fmt.Sprintf("xelora doc delete --all --kb=%s -y", opts.KB)); err != nil {
		return err
	}
