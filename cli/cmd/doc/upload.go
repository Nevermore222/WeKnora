...
// NewCmdUpload builds `xelora doc upload <file>`.
...
--kb flag > XELORA_KB_ID env > .xelora/project.yaml > error. The --kb
...
directory tree (see Examples). To ingest a remote URL use "xelora doc fetch";
to create an entry from inline text use "xelora doc create".
...
Server-side ingestion knobs:
  ...
  --channel <name>         Override the ingestion-channel tag (default "api").
                           Applies to file / --recursive.`,
		Example: `  xelora doc upload report.pdf
  xelora doc upload notes.md --kb a32a63ff-fb36-4874-bcaa-30f48570a694
  xelora doc upload notes.md --kb my-kb
  xelora doc upload q3.pdf --name "Q3 Marketing Report.pdf"
  xelora doc upload report.pdf --enable-multimodel --metadata team=alpha --metadata sprint=Q4
  xelora doc upload ./docs --recursive --glob '*.pdf' --metadata team=alpha`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor:       "Upload a local file to the resolved knowledge base. KB resolved via --kb flag, XELORA_KB_ID env, or project link. Emits the created Knowledge object with its id.",
...
func validateUploadFlags(opts *UploadOptions, args []string) error {
	...
	if !hasPath {
		...
		return cmdutil.NewFlagError(errors.New(
			"a file path is required (or use `xelora doc fetch` for URLs, `xelora doc create` for inline text)"))
	}
	return nil
}
