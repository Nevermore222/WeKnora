...
const agentEditExample = `  xelora agent edit ag_abc --name "Renamed" -y
  xelora agent edit ag_abc --description "" -y              # clear description
  xelora agent edit ag_abc --add-kb kb_new --remove-kb kb_old -y
  xelora agent edit ag_abc --system-prompt-file ./prompt.md -y
  xelora agent edit ag_abc --config-file ./tuned.yaml --temperature 0.9 -y`

// NewCmdEdit builds `xelora agent edit <agent-id>`.
func NewCmdEdit(f *cmdutil.Factory) *cobra.Command {
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			"xelora agent edit ag_abc --name \"Renamed\"",
			"xelora agent edit ag_abc --add-kb kb_new --remove-kb kb_old",
			"xelora agent edit ag_abc --config-file ./tuned.yaml",
		},
		Warnings: []string{
...
func buildAgentEditRetryCmd(cmd *cobra.Command, id string) string {
	var parts []string
	parts = append(parts, "xelora", "agent", "edit", id)
