// Package agentcmd holds the `xelora agent` command tree:
...
//
// "agent" in this subtree refers to Xelora's user-defined Custom
// Agents (server resource: GET/POST /agents/...) and handles CRUD
// operations only. Agent invocation has moved to `xelora session ask
...
// NewCmd builds the `xelora agent` parent and registers leaves. Called
...
or delete agents. To invoke an agent, use: xelora session ask --agent <id>`,
	}
