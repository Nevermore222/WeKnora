...
import (
	...
	mcpserver "github.com/Tencent/Xelora/cli/internal/mcp"
)

// NewCmdServe builds `xelora mcp serve`. Currently stdio-only; HTTP
...
Consult your MCP client's documentation for the exact config-file location.`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor: "run xelora as a long-lived MCP (Model Context Protocol) server over stdio for an IDE/host agent",
		Output:  "no stdout payload (JSON-RPC 2.0 protocol traffic); logs go to stderr",
		Warnings: []string{
			"this is a long-running stdio server, not a one-shot command — register it in your MCP client (command: xelora, args: [mcp, serve])",
