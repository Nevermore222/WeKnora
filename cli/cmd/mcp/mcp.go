// Package mcpcmd holds the `xelora mcp` command tree.
//
...
// `xelora mcp serve` exposes a curated subset of the CLI as MCP tools so
// an IDE-side agent can list / view / search / chat against the user's
// active Xelora profile without shelling out to the CLI per call. Most
...
// NewCmd builds the `xelora mcp` parent. Called from cli/cmd/root.go.
func NewCmd(f *cmdutil.Factory) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "mcp",
		Short: "Run xelora as a Model Context Protocol server",
		Long: `Exposes xelora's tool surface as MCP tools so any
