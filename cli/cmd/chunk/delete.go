...
const chunkDeleteExample = `  xelora chunk delete chunk_abc --doc doc_xyz                  # interactive confirm
  xelora chunk delete chunk_abc --doc doc_xyz -y               # no prompt
  xelora chunk delete chunk_abc --doc doc_xyz -y --format json # bare {id, deleted:true} JSON
  xelora chunk delete c1 c2 c3 --doc doc_xyz -y                # delete 3 chunks under same doc, keep-going`

// NewCmdDelete builds `xelora chunk delete <chunk-id> [<chunk-id>...] --doc <doc-id>`.
func NewCmdDelete(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if err := cmdutil.ConfirmDestructiveBatch(f.Prompter(), opts.Yes, fopts.WantsJSON(), "delete", "chunk", len(args), "chunk.delete", "xelora chunk delete "+strings.Join(args, " ")+" --doc "+opts.DocID+" -y"); err != nil {
				return err
			}
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			"xelora chunk delete chunk_abc --doc doc_xyz -y",
			"xelora chunk delete c1 c2 c3 --doc doc_xyz -y",
			"xelora chunk delete chunk_abc --doc doc_xyz -y --format json",
		},
		Warnings: []string{
...
func runDelete(ctx context.Context, opts *DeleteOptions, fopts *cmdutil.FormatOptions, svc DeleteService, p prompt.Prompter) error {
	if err := cmdutil.ConfirmDestructive(p, opts.Yes, fopts.WantsJSON(), "delete", "chunk", opts.ChunkID, "chunk.delete", "xelora chunk delete "+opts.ChunkID+" --doc "+opts.DocID+" -y"); err != nil {
		return err
	}
