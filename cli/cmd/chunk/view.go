...
less or use --format json for large chunks. Xelora chunks are typically bounded
...
const chunkViewExample = `  xelora chunk view chunk_abc
  xelora chunk view chunk_abc --format json | jq '.content'
  xelora chunk view chunk_abc --format json --jq '{id, chunk_index, is_enabled}'`

// NewCmdView builds `xelora chunk view <chunk-id>`.
func NewCmdView(f *cmdutil.Factory) *cobra.Command {
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples:      []string{"xelora chunk view chunk_abc"},
