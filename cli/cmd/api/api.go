// Package api implements the `xelora api` raw HTTP passthrough command.
//
...
// NewCmd returns the `xelora api` command.
func NewCmd(f *cmdutil.Factory) *cobra.Command {
	opts := &Options{}
	cmd := &cobra.Command{
		Use:   "api <path>",
		Short: "Make a raw API request to the Xelora server",
...
Examples:
  xelora api /api/v1/knowledge-bases                                # GET
  echo '{"name":"foo"}' | xelora api /api/v1/knowledge-bases --input -  # POST (auto)
  xelora api /api/v1/knowledge-bases/kb_xxx -X DELETE`,
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			method := resolveMethod(opts)
			// Escape-hatch DELETE through `xelora api` is just as destructive
			// as `xelora kb delete` - exit-10 protocol must apply (cli/README.md).
			if method == http.MethodDelete {
				if err := cmdutil.ConfirmDestructive(f.Prompter(), opts.Yes, fopts.WantsJSON(), "delete", "endpoint", args[0], "api.delete", "xelora api -X DELETE "+args[0]+" -y"); err != nil {
					return err
				}
			}
			cli, err := f.Client()
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor:       "raw HTTP passthrough to xelora-server API endpoints when typed subcommands are insufficient",
		RequiredFlags: []string{"path (positional)"},
		Examples: []string{
			"xelora api /api/v1/knowledge-bases",
			"xelora api -X DELETE /api/v1/knowledge-bases/kb_x -y",
			"echo '{\"name\":\"foo\"}' | xelora api -X POST /api/v1/knowledge-bases --input -",
		},
