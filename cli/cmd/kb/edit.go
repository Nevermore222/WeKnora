...
// NewCmdEdit builds `xelora kb edit <id>`. At least one of --name /
...
to the user first.`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			"xelora kb edit kb_abc --name \"New Name\" -y",
			"xelora kb edit kb_abc --description \"Updated desc\" --format json -y",
		},
		Warnings: []string{
...
func buildKBEditRetryCmd(c *cobra.Command, id string) string {
	var parts []string
	parts = append(parts, "xelora", "kb", "edit", id)
