...
import (
	...
	agentcmd "github.com/Tencent/Xelora/cli/cmd/agent"
	apicmd "github.com/Tencent/Xelora/cli/cmd/api"
	"github.com/Tencent/Xelora/cli/cmd/auth"
	chatcmd "github.com/Tencent/Xelora/cli/cmd/chat"
	chunkcmd "github.com/Tencent/Xelora/cli/cmd/chunk"
	"github.com/Tencent/Xelora/cli/cmd/doc"
	"github.com/Tencent/Xelora/cli/cmd/doctor"
	"github.com/Tencent/Xelora/cli/cmd/kb"
	linkcmd "github.com/Tencent/Xelora/cli/cmd/link"
	mcpcmd "github.com/Tencent/Xelora/cli/cmd/mcp"
	profilecmd "github.com/Tencent/Xelora/cli/cmd/profile"
	"github.com/Tencent/Xelora/cli/cmd/search"
	sessioncmd "github.com/Tencent/Xelora/cli/cmd/session"
	"github.com/Tencent/Xelora/cli/internal/build"
...
func resolveFormatEarly(args []string) {
	...
	if mode == "" {
		if v := os.Getenv("XELORA_FORMAT"); v != "" {
...
func NewRootCmd(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "xelora",
		Short: "Xelora CLI",
		Long: `Command-line client for the Xelora RAG server. Manage knowledge bases
and documents, run hybrid search, chat with grounded answers, or expose
a curated read-only MCP tool surface for AI agents.`,
		Example: `  xelora profile add prod --host=https://kb.example.com --use
  xelora auth login
  xelora kb list
  xelora chat "summarise the design doc"
  xelora doctor --format json`,
		SilenceUsage:  true,
		SilenceErrors: true,
		...
		PersistentPreRunE: func(c *cobra.Command, args []string) error {
			// Propagate the global --profile flag (or XELORA_PROFILE env) into
			...
			if v, _ := c.Flags().GetString("profile"); v != "" {
				f.ProfileOverride = v
			} else if v := os.Getenv("XELORA_PROFILE"); v != "" {
				f.ProfileOverride = v
			}
			...
			// Resolve --log-level / XELORA_LOG_LEVEL and apply to the SDK
	...
	// Match `xelora version` line format so both forms output the same.
	cmd.SetVersionTemplate("xelora {{.Version}}\n")
	addGlobalFlags(cmd)
...
func addGlobalFlags(cmd *cobra.Command) {
	...
	// (e.g. `xelora fooo --format json`) reach the typed-envelope guard
...
func newVersionCmd(f *cmdutil.Factory) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			fmt.Fprintf(c.OutOrStdout(), "xelora %s (commit %s, built %s)\n", v, commit, date)
			return nil
		},
	}
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{"xelora version --format json"},
...
// (e.g. `xelora kb bogus`). Without this, cobra falls back to a free-form
...
func unknownSubcommandRunE(cmd *cobra.Command, args []string) error {
	// Group command invoked with no subcommand (e.g. `xelora kb`):
