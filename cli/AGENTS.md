...
This is the Xelora CLI (`xelora`), a command-line client for the Xelora RAG server. The module path is `github.com/Tencent/Xelora/cli`.

The wire contract for AI agents *consuming* `xelora` output (JSON shape, exit codes, error format) is documented below and in [README.md](README.md). Read this file if you're integrating with the CLI binary — build / test / architecture details follow the wire contract sections.
...
```json
{
  ...
  "error": {
    ...
    "retry_command": "xelora auth login",
...
| Variable | Purpose |
|---|---|
| `XELORA_PROFILE` | Active profile name. Equivalent to the global `--profile <name>` flag; overridden by `--profile`. Useful in CI scripts that cannot pass global flags. |
| `XELORA_FORMAT` | Default `--format` value (`text \| json \| ndjson`). Overridden by explicit `--format`. Invalid values ignored silently. |
| `XELORA_KB_ID` | Default KB ID for commands that accept `--kb`. Overridden by `--kb`. |
| `XELORA_LOG_LEVEL` | SDK debug log level (`error \| warn \| info \| debug`). Overridden by `--log-level`. |
| `XELORA_AGENT_HELP` | Set to `1` to emit structured JSON agent-help (machine-readable) instead of human help text when `--help` is invoked. |
...
> subcommand (`xelora version --format json`) or `XELORA_AGENT_HELP=1
> xelora <cmd> --help`. Planned fix for v0.8.

## Design decisions worth flagging

Five design decisions readers may want context on: where Xelora picks an
...
| | |
|---|---|
| **Xelora** | success envelope → stdout; error envelope → stderr |
| **Rationale** | `xelora ... --format json \| jq '.data[]'` must not mix error objects into the data stream. Channel split lets pipeline consumers suppress errors with `2>/dev/null` and still get clean JSON on stdout. |

### 2. `xelora api DELETE` triggers exit-10 confirmation

| | |
|---|---|
| **Xelora** | DELETE triggers exit-10 (`input.confirmation_required`); user bypasses with `-y/--yes` |
...
| | |
|---|---|
| **Xelora** | two separate fields: `retry_command` (suggested next argv, directly-executable for non-destructive errors; informational only on exit-10) + `hint` (prose) |
...
| | |
|---|---|
| **Xelora** | streaming commands (`chat`, `session ask`) emit bare `{type:...}` per line; no envelope |
...
| **Xelora** | version identity via CLI binary semver + CHANGELOG `### BREAKING` + skill `tested_against` + CI parity tests |
| **Rationale** | Mainstream CLIs don't embed version in payload. Agents have complete version awareness via `xelora --version` and skill version binding. |
...
```bash
go build -o xelora .                              # build (from cli/)
...
A command `xelora foo bar` lives in `cmd/foo/bar.go` with `bar_test.go`.
...
Use a Go raw string with `xelora` as the example prefix. Keep one-line `Short` ≤ 70 chars; `Long` may run multi-paragraph; `Example` always includes `xelora` so copy-paste works:

```go
Example: `  xelora kb view <id>
  xelora kb view kb_abc --format json
  xelora kb view kb_abc --format json --jq '{id, name}'`,
```
...
> **Audience:** AI agents and scripted callers parsing `xelora` stderr.
> Code authors writing new error sites — see [`## Error Handling`](#error-handling) above.

When `xelora` exits non-zero, stderr carries a structured triplet:
...
|---|---|---|---|
| `auth.unauthenticated` | 3 | no (run `auth login`) | run `xelora auth login` |
| `auth.token_expired` | 3 | yes (after refresh) | your session expired; run `xelora auth login` to re-authenticate |
| `auth.bad_credential` | 3 | no (re-login) | run `xelora auth login` |
| `auth.forbidden` | 3 | no | active profile lacks permission for this resource |
| `auth.cross_tenant_blocked` | 3 | no | verify tenant context with `xelora auth status` |
| `auth.tenant_mismatch` | 3 | no | verify tenant context with `xelora auth status` |
| `input.invalid_argument` | 5 | no | see `xelora <command> --help` for valid usage |
| `input.missing_flag` | 5 | no | see `xelora <command> --help` for valid usage |
...
| `server.timeout` | 7 | yes (with backoff) | request timed out; retry, or run `xelora doctor` to check connectivity |
...
| `server.incompatible_version` | 7 | no (upgrade required) | run `xelora doctor` to see version skew details |
| `network.error` | 7 | yes (with backoff) | check base URL reachability with `xelora doctor` |
...
| `local.config_corrupt` | 1 | no (manual fix) | remove `~/.config/xelora/config.yaml` and re-run `xelora auth login` |
| `local.profile_not_found` | 1 | no | (no canonical hint; check `xelora profile list`) |
| `local.file_io` | 1 | no | check file permissions under `$XDG_CONFIG_HOME/xelora/` |
| `local.kb_id_required` | 1 | no | run `xelora link` to bind this directory to a knowledge base, or pass `--kb` |
| `local.kb_not_found` | 1 | no | list available with `xelora kb list` |
| `local.keychain_denied` | 1 | no (system-level) | verify keyring access; falls back to file storage |
| `local.project_link_corrupt` | 1 | no | remove `.xelora/project.yaml` and run `xelora link` again |
...
- `auth.token_expired` → run `xelora auth refresh`, then retry once
...
The `xelora session continue-stream <session-id> --message <msg-id>` command resumes an SSE event stream for an existing assistant message. Use cases: network-blip recovery, long-running agent invocation polling, completed-stream inspection.
...
After TTL, `xelora session continue-stream` returns the typed error `local.sse_stream_aborted`, which maps to exit code 1 per the Error code reference.

## Dry-run contract

The `--dry-run` flag is available on every mutation cobra command (`kb create/edit/delete`, `agent create/edit/delete`, `doc create/upload/fetch/delete`, `chunk delete`, `session delete`, `auth refresh/logout`, `link/unlink`, `profile add/remove`) and on `xelora api` (POST/PUT/PATCH/DELETE only; GET rejected with FlagError exit 2).
...
The dry-run path is **offline** — no SDK calls, no Factory.Client() init, no ResolveKB network query, no writes (keyring / `.xelora/project.yaml` / files). Works without active profile or network access.
...
```bash
echo '{"query":"...","kb":"..."}' | xelora api -X POST /api/v1/sessions/<id>/agent-qa --input - --dry-run
```
...
2. **Help text** — a `Risk: <action> (destructive)` line prepended to the top of `--help` output on the 9 destructive cobra commands. `xelora api` is intentionally excluded: it is a generic HTTP passthrough whose risk depends on the method, so a static Risk: line would mislead for non-DELETE methods.

3. **MCP tool annotations** — `Tool.Annotations.destructiveHint` / `readOnlyHint` / `idempotentHint` / `openWorldHint` on every tool returned by `xelora mcp serve`.
...
Xelora's MCP server exposes a curated 10-tool surface where most tools are read-only but `chat` and `session_ask` create conversation/message records. Many MCP servers in the wild ship write / mutation operations on by default and rely on credential-scope or sandbox restrictions for safety. Xelora opts for curation instead: the server side doesn't yet enforce per-token scope, so an agent holding a user's token has full write access. Until server-side scope ships, the CLI keeps mutation tools out of the MCP surface as a belt-and-braces second line of defense. When server scope arrives this stance can loosen.
...
6. Decide which fields are "top use case" (flag) / "advanced" (`--config-file` or escape hatch via `xelora api`). Don't try to flag-cover every SDK field — mature CLIs that curate ship a tighter surface; CLIs that 1:1 mirror their API pay the UX cost.
...
- Consistent with every existing non-auth Xelora command.
...
- **Agent help blob**: Commands MAY call
  ...
  Activated by `XELORA_AGENT_HELP=1` at `--help` time. Warnings are
