...
func main() {
	...
	FileMenu.AddText("About Xelora", keys.CmdOrCtrl("i"), func(_ *menu.CallbackData) {
		...
		if choice == "Open GitHub" {
			wailsruntime.BrowserOpenURL(app.ctx, xeloraGitHubRepoURL)
		}
	})
	...
	err := wails.Run(&options.App{
		...
		OnDomReady: func(ctx context.Context) {
			...
			if u := strings.TrimSpace(app.backendURL); u != "" {
				...
				inject := fmt.Sprintf(`try{window.__XELORA_API_BASE__=%s}catch(e){}`, strconv.Quote(apiRoot))
				wailsruntime.WindowExecJS(ctx, inject)
			}
			if lan := strings.TrimSpace(app.apiLanBaseURL); lan != "" {
				injectLan := fmt.Sprintf(`try{window.__XELORA_API_LAN_BASE__=%s}catch(e){}`, strconv.Quote(lan))
...
func configureDesktopStorage(execPath string) {
	...
	dbPath := resolveDesktopDataPath(os.Getenv("DB_PATH"), filepath.Join("data", "xelora.db"), appSupportDir)
