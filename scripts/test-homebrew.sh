...
TAP_NAME="xelora/test"
FORMULA_NAME="xelora-lite-test"
VERSION="test"
GOOS=$(go env GOOS)
GOARCH=$(go env GOARCH)
ARCHIVE="Xelora-lite_${VERSION}_${GOOS}_${GOARCH}"
...
TAP_DIR="$(brew --repository)/Library/Taps/xelora/homebrew-test"
if [ ! -d "${TAP_DIR}" ]; then
...
class WeknoraLiteTest < Formula
  ...
  def install
    ...
    (bin/"xelora-lite-test").write <<~SH
      #!/bin/bash
      CONFIG_DIR="\\\${XELORA_CONFIG_DIR:-\\\${XDG_CONFIG_HOME:-\\\$HOME/.config}/xelora-test}"
      DATA_DIR="\\\${XELORA_DATA_DIR:-\\\${XDG_DATA_HOME:-\\\$HOME/.local/share}/xelora-test}"
      ...
      if [ ! -f "\\\$CONFIG_DIR/.env.lite" ]; then
        ...
        sed -i'' -e "s|DB_PATH=.*|DB_PATH=\\\$DATA_DIR/xelora.db|" "\\\$CONFIG_DIR/.env.lite"
  ...
  def post_install
    (var/"xelora-test").mkpath
    (var/"log").mkpath
  end

  service do
    run [bin/"xelora-lite-test"]
    keep_alive true
    working_dir var/"xelora-test"
    log_path var/"log/xelora-lite-test.log"
    error_log_path var/"log/xelora-lite-test.log"
  end

  test do
    assert_predicate bin/"xelora-lite-test", :executable?
  end
end
RUBY
...
echo ""
echo "  which:"
which xelora-lite-test || true
echo ""
echo "  Installed files:"
brew list "${TAP_NAME}/${FORMULA_NAME}"
echo ""
echo "  Test paths (isolated from production):"
echo "    Config: ~/.config/xelora-test/.env.lite"
echo "    Data:   ~/.local/share/xelora-test/"

echo ""
echo "=== Done ==="
echo ""
echo "前台运行:"
echo "  xelora-lite-test"
echo ""
echo "后台服务:"
