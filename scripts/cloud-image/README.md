# Xelora 云镜像打包脚本（Cloud-Agnostic）

> **本文档面向「想把 Xelora 打包成云镜像（AMI / 自定义镜像 / Snapshot）分发给其他人」的用户。**
> **如果你只是想自己用 Xelora，请直接看主仓 [README](../../README.md)，`docker compose up -d` 即可。**
...
- 别人基于这份镜像创建新实例后，**首次开机会自动**：
  ...
  - 启动 Xelora 全部默认容器
  - 把生成的凭证写到 `/root/xelora-credentials.txt`
  - 自删除一次性初始化脚本
...
└── systemd/
    ├── xelora.service           # 开机自启 docker compose
    └── xelora-firstboot.service # 首次启动 init(执行后自删)
```

## 不需要 clone 整个 Xelora 仓库

Xelora 所有容器都从 Docker Hub 拉镜像（`wechatopenai/xelora-*`），Go / Python / 前端源码都不需要带到宿主机。
...
Xelora `docker-compose.yml` 大量服务是 **profile 限定**，本镜像只默认启动核心 5 个。
...
**1. 拷入脚本（任选一种，都不用 clone 整个 Xelora 仓库）。**
...
GH_PROXY="${GH_PROXY:-}"
mkdir -p /opt/xelora-tools && cd /opt/xelora-tools
git init -q && git remote add origin "${GH_PROXY}https://github.com/Tencent/Xelora.git"
...
# 不通时设 GH_PROXY=https://gh-proxy.com/ 或 https://ghfast.top/, 注意末尾斜杠。
GH_PROXY="${GH_PROXY:-}"
mkdir -p /opt/xelora-tools/scripts/cloud-image/systemd && cd /opt/xelora-tools
base="${GH_PROXY}https://raw.githubusercontent.com/Tencent/Xelora/main/scripts/cloud-image"
for f in prepare.sh cleanup.sh firstboot.sh README.md; do
  curl -fsSL "$base/$f" -o "scripts/cloud-image/$f"
done
for f in xelora.service xelora-firstboot.service; do
...
#   scp -r scripts/cloud-image root@<实例IP>:/opt/xelora-tools/scripts/
```
...
```bash
sudo bash /opt/xelora-tools/scripts/cloud-image/prepare.sh

# 想 pin 特定版本（推荐, 保证镜像可复现）
sudo XELORA_REF=v0.5.0 bash /opt/xelora-tools/scripts/cloud-image/prepare.sh

# 中国大陆机器三件套: 同时绕开 GitHub / get.docker.com / Docker Hub 的境外 CDN
# (以腾讯云为例, 阿里云 / 华为云换对应镜像即可)
sudo \
  XELORA_REF=v0.5.0 \
  XELORA_GH_PROXY=https://gh-proxy.com/ \
  ...
  bash /opt/xelora-tools/scripts/cloud-image/prepare.sh
```

> 三个变量分别解决三个不同的境外 CDN 不可达问题:
> - `XELORA_GH_PROXY`：加速 **GitHub tarball** 下载（`prepare.sh` 步骤 2，运行时文件）
> - `DOCKER_INSTALL_MIRROR`：绕开 **`get.docker.com`**，改用 apt + docker-ce 镜像源装 Docker（步骤 1）
> - `DOCKER_REGISTRY_MIRROR`：加速 **Docker Hub** 镜像拉取（步骤 4，`wechatopenai/xelora-*`）
>
...
2. 用 `curl + tar` 下载 4 个运行时文件到 `/opt/Xelora`
3. 拉取并启动默认 5 个容器 + 预拉 sandbox 镜像
4. 安装 `xelora.service`（开机自启）+ `xelora-firstboot.service`（首启 init）
...
```bash
sudo docker compose -f /opt/Xelora/docker-compose.yml ps
...
```bash
sudo bash /opt/xelora-tools/scripts/cloud-image/cleanup.sh
```
...
```bash
sudo XELORA_REF=v0.6.0 bash /opt/xelora-tools/scripts/cloud-image/prepare.sh
sudo bash    /opt/xelora-tools/scripts/cloud-image/cleanup.sh   # 制作新镜像前
```

> **打新镜像时想顺便清掉旧版本镜像层**（每个旧 xelora-* tag 几百 MB，4 个镜像 ~2-4GB）：
> ```bash
> sudo PRUNE_OLD_IMAGES=true XELORA_REF=v0.6.0 \
>   bash /opt/xelora-tools/scripts/cloud-image/prepare.sh
> ```
...
- `/root/xelora-credentials.txt` 用 `umask 077` 创建，仅 root 可读
