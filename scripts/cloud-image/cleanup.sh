...
echo "[cleanup] 1/8 停止 Xelora 容器"
COMPOSE_PROJECT=""
if [[ -d "${XELORA_DIR}" ]]; then
  cd "${XELORA_DIR}"
  # 优先用 compose ls 拿到真实 project 名 (默认是目录名小写, 如 xelora)
  COMPOSE_PROJECT="$(docker compose ls --format json 2>/dev/null \
    | grep -oE '"Name":"[^"]+"' | head -1 | cut -d'"' -f4 || true)"
  docker compose down -v --remove-orphans || true
fi

echo "[cleanup] 2/8 清空 Xelora 业务数据 + 首启 marker / 日志"
if [[ -d "${XELORA_DIR}" ]]; then
  rm -rf "${XELORA_DIR}/data"/* "${XELORA_DIR}/logs"/* 2>/dev/null || true
  ...
  rm -f "${XELORA_DIR}/.env" "${XELORA_DIR}/.firstboot.done"
fi
rm -f /root/xelora-credentials.txt /var/log/xelora-firstboot.log
...
# wechatopenai/xelora-* 等镜像一并清掉, 导致基于镜像建出来的新实例
