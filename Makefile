...
help:
	...
	@echo ""
	@echo "Docker 命令:"
	@echo "  docker-build-app       构建应用 Docker 镜像 (wechatopenai/xelora-app)"
	@echo "  docker-build-docreader 构建文档读取器镜像 (wechatopenai/xelora-docreader)"
	@echo "  docker-build-frontend  构建前端镜像 (wechatopenai/xelora-ui)"
...
DOCKER_IMAGE=wechatopenai/xelora-app
DOCKER_TAG=latest
...
docker-build-docreader:
	docker build --platform $(PLATFORM) -f docker/Dockerfile.docreader -t wechatopenai/xelora-docreader:latest .

# Build frontend Docker image
docker-build-frontend:
	./scripts/build_frontend_dist.sh
	docker build --platform $(PLATFORM) -f frontend/Dockerfile -t wechatopenai/xelora-ui:latest frontend/
...
build-prod:
	...
	LDFLAGS="-X 'github.com/Tencent/Xelora/internal/handler.Version=$$VERSION' -X 'github.com/Tencent/Xelora/internal/handler.Edition=standard' -X 'github.com/Tencent/Xelora/internal/handler.CommitID=$$COMMIT_ID' -X 'github.com/Tencent/Xelora/internal/handler.BuildTime=$$BUILD_TIME' -X 'github.com/Tencent/Xelora/internal/handler.GoVersion=$$GO_VERSION' -X 'google.golang.org/protobuf/reflect/protoregistry.conflictPolicy=warn'"; \
...
clean-db:
	...
	@if [ $$(docker volume ls -q -f name=xelora_postgres-data) ]; then \
		docker volume rm xelora_postgres-data; \
	fi
	@if [ $$(docker volume ls -q -f name=xelora_minio_data) ]; then \
		docker volume rm xelora_minio_data; \
	fi
	@if [ $$(docker volume ls -q -f name=xelora_redis_data) ]; then \
		docker volume rm xelora_redis_data; \
	fi
