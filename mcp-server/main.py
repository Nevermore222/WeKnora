#!/usr/bin/env python3
"""
Xelora MCP Server 主入口点

这个文件提供了一个统一的入口点来启动 Xelora MCP 服务器。
可以通过多种方式运行：
1. python main.py
2. python -m xelora_mcp_server
3. xelora-mcp-server (安装后)
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Xelora MCP Server - Model Context Protocol server for Xelora API",
...
环境变量:
  XELORA_BASE_URL    Xelora API 基础 URL (默认: http://localhost:8080/api/v1)
  XELORA_API_KEY     Xelora API 密钥
        """,
    )
    ...
    parser.add_argument(
        "--version", action="version", version="Xelora MCP Server 1.0.0"
    )
...
async def main():
    ...
    try:
        print(f"正在启动 Xelora MCP Server (transport={args.transport})...")

        from xelora_mcp_server import run_stdio, run_sse, run_http
