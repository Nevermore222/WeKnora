...
XELORA_BASE_URL = os.getenv("XELORA_BASE_URL", "http://localhost:8080/api/v1")
XELORA_API_KEY = os.getenv("XELORA_API_KEY", "")
# Chat SSE read timeout in seconds. LLM responses can be slow; default 300s.
try:
    XELORA_CHAT_TIMEOUT = int(os.getenv("XELORA_CHAT_TIMEOUT", "300"))
except ValueError:
    logger.warning("XELORA_CHAT_TIMEOUT is not a valid integer; falling back to 300s.")
    XELORA_CHAT_TIMEOUT = 300


class XeloraClient:
    """Client for interacting with Xelora API"""

    def __init__(self, base_url: str, api_key: str):
        """Initialize the Xelora API client with base URL and authentication"""
        self.base_url = base_url
        self.api_key = api_key
        # SSL verification: enabled by default. Set XELORA_VERIFY_SSL=false to disable
        # (e.g. for self-signed certs in dev environments — NOT recommended for production).
        self.verify_ssl = os.getenv("XELORA_VERIFY_SSL", "true").lower() != "false"
        if not self.verify_ssl:
            logger.warning(
                "SSL certificate verification is DISABLED (XELORA_VERIFY_SSL=false). "
    ...
    def _request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        """Make a request to the Xelora API
    ...
    def _consume_sse_stream(self, url: str, body: Dict[str, Any]) -> Dict:
        ...
        Timeout: (10s connect, XELORA_CHAT_TIMEOUT read) — configurable via env var.
        ...
        try:
            ...
            # Timeout: 10s to establish connection, XELORA_CHAT_TIMEOUT for reading response
            response = self.session.post(
                url, json=body, stream=True,
                timeout=(10, XELORA_CHAT_TIMEOUT),
            )
            response.raise_for_status()
...
app = Server("xelora-server")
# Initialize Xelora API client with configuration
client = XeloraClient(XELORA_BASE_URL, XELORA_API_KEY)


# Tool definitions - Register all available tools for the MCP protocol
@app.list_tools()
async def handle_list_tools() -> list[types.Tool]:
    """List all available Xelora tools with their schemas"""
    return [
        # Tenant Management
        types.Tool(
            name="create_tenant",
            description="Create a new tenant in Xelora",
            inputSchema={
                "type": "object",
                "properties": {
...
def main():
    ...
    Transport selection (in priority order):
      ...
      3. Default: stdio
    """
    parser = argparse.ArgumentParser(description="Xelora MCP Server")
    parser.add_argument(
        "--transport",
