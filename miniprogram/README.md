...
This directory contains a WeChat Mini Program plugin for Xelora. It gives mobile users a lightweight entry point to:

- configure a Xelora API endpoint and tenant API key;
- list available knowledge bases;
- import a URL into a selected knowledge base;
- ask a selected knowledge base through Xelora knowledge chat.
...
3. Open the **Settings** tab and fill in:
   - API Base URL, for example `https://xelora.example.com`;
   - API Key from the Xelora tenant settings page.
...
- WeChat DevTools may block `localhost` requests when URL validation is enabled. For local testing, either disable domain validation in DevTools or expose Xelora through a HTTPS development domain.
- In production Mini Programs, add the Xelora API domain to the Mini Program request domain allowlist.
