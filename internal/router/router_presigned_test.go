...
func (s *stubTenantService) GetXeloraCloudCredentials(context.Context) *types.XeloraCloudCredentials {
	panic("unexpected")
}
...
func setupPresignedTestServer(t *testing.T) (engine *gin.Engine, baseDir string, signURL func(filePath string, tenantID uint64, ttl time.Duration) string) {
	...
	t.Setenv("SYSTEM_AES_KEY", "xelora-test-aes-key-32bytes!!!")
	...
	signURL = func(filePath string, tenantID uint64, ttl time.Duration) string {
		signed, err := secutils.SignFileURL("https://xelora.example.com", filePath, tenantID, ttl)
		if err != nil {
