...
// Enabled when XELORA_LLM_STREAM_RAW_DUMP_DIR is set, or when
// XELORA_LLM_STREAM_RAW_DUMP=1 (defaults to ~/.xelora/investigate/llm-stream).
func streamRawDumpDir() string {
	if dir := strings.TrimSpace(os.Getenv("XELORA_LLM_STREAM_RAW_DUMP_DIR")); dir != "" {
		return dir
	}
	v := strings.TrimSpace(os.Getenv("XELORA_LLM_STREAM_RAW_DUMP"))
	if v == "1" || strings.EqualFold(v, "true") || strings.EqualFold(v, "yes") {
		home, err := os.UserHomeDir()
		if err != nil {
			return ""
		}
		return filepath.Join(home, ".xelora", "investigate", "llm-stream")
	}
	return ""
}
