...
func (g *Gateway) RunSkillScriptJob(ctx context.Context, req SkillJobRequest, executor SkillExecutor) (*SkillJobExecution, error) {
	...
	if snapshotErr == nil {
		artifacts := detectArtifacts(basePath, job.ID, req.SessionID, preSnapshot, postSnapshot, outcome.MaterializedInputPaths)
...
func detectArtifacts(basePath, jobID, sessionID string, pre, post map[string]fileSnapshot, materializedInputPaths []string) []*ArtifactRecord {
...
func isArtifactFile(path string) bool {
	...
	case ".md", ".html", ".pdf", ".csv", ".xlsx", ".xlsm", ".pptx", ".docx", ".png", ".jpg", ".jpeg", ".webp", ".svg":
		return true
	default:
		return false
	}
