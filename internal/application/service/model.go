...
// resolveXeloraCloudCredentials 为 XeloraCloud 厂商模型补全 AppID/AppSecret。
// 当模型自身参数中未存储凭证时，自动从租户配置中获取（SaveCredentials 保存的凭证）。
func (s *modelService) resolveXeloraCloudCredentials(ctx context.Context, params *types.ModelParameters) (appID, appSecret string) {
	appID = params.AppID
	appSecret = s.decryptAppSecret(params.AppSecret)

	if provider.ProviderName(params.Provider) != provider.ProviderXeloraCloud {
		return
	}
	if appID != "" && appSecret != "" {
		return
	...
	creds := s.tenantService.GetXeloraCloudCredentials(ctx)
	if creds == nil {
		return
	}
	if appID == "" {
...
func (s *modelService) GetEmbeddingModel(ctx context.Context, modelId string) (embedding.Embedder, error) {
	...
	appID, appSecret := s.resolveXeloraCloudCredentials(ctx, &model.Parameters)
