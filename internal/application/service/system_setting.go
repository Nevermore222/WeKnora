...
// suffix XELORA_REDIS_NAMESPACE so two deployments sharing one Redis
// instance don't cross-talk.
const pubsubChannelBase = "xelora:system_settings:changed"
...
func pubsubChannel() string {
	if ns := strings.TrimSpace(os.Getenv("XELORA_REDIS_NAMESPACE")); ns != "" {
...
var registry = map[string]settingSpec{
	...
	// EnvName is the same XELORA_TENANT_MAX_OWNED_PER_USER that
	...
	"tenant.max_owned_per_user": {
		Type:     "int",
		EnvName:  "XELORA_TENANT_MAX_OWNED_PER_USER",
		Default:  int64(10),
		Category: "tenant",
	...
	"tenant.default_storage_quota_gb": {
		Type:     "int",
		EnvName:  "XELORA_TENANT_DEFAULT_STORAGE_QUOTA_GB",
		Default:  int64(10),
		Category: "tenant",
	...
	// XELORA_ASYNQ_CONCURRENCY (default 32).
	"asynq.concurrency": {
		Type:            "int",
		EnvName:         "XELORA_ASYNQ_CONCURRENCY",
		Default:         int64(32),
		Category:        "worker",
