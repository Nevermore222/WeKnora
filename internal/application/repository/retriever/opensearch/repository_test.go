...
func TestIndexAlias(t *testing.T) {
	t.Parallel()
	r := &Repository{baseIndex: "xelora_abc123def456"}
	if got := r.indexAlias(768); got != "xelora_abc123def456_768" {
		t.Errorf("indexAlias(768): want xelora_abc123def456_768, got %s", got)
	}
}
...
func TestKeywordsIndex(t *testing.T) {
	t.Parallel()
	r := &Repository{baseIndex: "xelora_abc123def456"}
	if got := r.keywordsIndex(); got != "xelora_abc123def456_keywords" {
		t.Errorf("keywordsIndex: want xelora_abc123def456_keywords, got %s", got)
	}
}
