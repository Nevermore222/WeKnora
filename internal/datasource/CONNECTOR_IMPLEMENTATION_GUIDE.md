...
A connector is an adapter that translates between Xelora's data model and an external platform's API. It handles:
...
func (c *YourConnector) ListResources(ctx context.Context, config *types.DataSourceConfig) ([]types.Resource, error) {
    ...
    // Convert to Xelora Resource format
...
import (
    // ... existing imports
    yourconnector "github.com/Tencent/Xelora/internal/datasource/connector/yourtype"
)
...
2. Xelora already has `internal/im/feishu/` for reference
