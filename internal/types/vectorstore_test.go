...
func TestBuildEnvVectorStores(t *testing.T) {
	envMap := map[string]string{
		...
		"TENCENT_VECTORDB_DATABASE":   "xelora",
		"TENCENT_VECTORDB_COLLECTION": "xelora_embeddings",
		"WEAVIATE_HOST":               "weaviate:8080",
		"DORIS_ADDR":                  "doris-fe:9030",
		"DORIS_HTTP_PORT":             "8030",
		"DORIS_DATABASE":              "xelora",
		"DORIS_USERNAME":              "root",
		"DORIS_PASSWORD":              "doris-pass",
		"DORIS_TABLE_PREFIX":          "xelora_embeddings",
	}
	...
	t.Run("tencent vectordb env store", func(t *testing.T) {
		...
		assert.Equal(t, "xelora", stores[0].ConnectionConfig.Database)
		assert.Equal(t, "xelora_embeddings", stores[0].IndexConfig.CollectionName)
	})
	...
	t.Run("doris env store", func(t *testing.T) {
		...
		assert.Equal(t, "xelora_embeddings", stores[0].IndexConfig.CollectionPrefix)
	})
...
func TestResolveCollectionName(t *testing.T) {
	...
	t.Run("empty IndexConfig falls back to default", func(t *testing.T) {
		...
		ic := &IndexConfig{}
		assert.Equal(t, "xelora_embeddings", ResolveCollectionName(ic, "QDRANT_COLLECTION", "xelora_embeddings"))
	})
}
...
func TestValidateIndexConfig(t *testing.T) {
	...
	t.Run("doris GetIndexNameOrDefault falls back when prefix empty", func(t *testing.T) {
		ic := IndexConfig{}
		assert.Equal(t, "xelora_embeddings", ic.GetIndexNameOrDefault(DorisRetrieverEngineType))
	})
