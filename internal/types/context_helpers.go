...
// EnvLanguage returns the XELORA_LANGUAGE environment variable value, or empty string if unset.
func EnvLanguage() string {
	return strings.TrimSpace(os.Getenv("XELORA_LANGUAGE"))
}

// DefaultLanguage returns the configured default language locale.
// It reads the XELORA_LANGUAGE environment variable; if unset, falls back to "zh-CN".
...
// Falls back to DefaultLanguage() (XELORA_LANGUAGE env, then "zh-CN").
